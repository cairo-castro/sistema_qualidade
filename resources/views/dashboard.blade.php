<x-app-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-600" x-data="{ count: {{ $totalDiagnosticos ?? 0 }} }">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-sm font-medium text-slate-600 dark:text-slate-300">Total de Diagnósticos</h3>
                    <p class="text-3xl font-bold text-slate-900 dark:text-white mt-2" x-text="formatNumber(count)">{{ number_format($totalDiagnosticos ?? 0) }}</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-600" x-data="{ rate: {{ $taxaConformidade ?? 85.4 }} }">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-sm font-medium text-slate-600 dark:text-slate-300">Taxa de Conformidade</h3>
                    <p class="text-3xl font-bold text-slate-900 dark:text-white mt-2" x-text="rate + '%'">{{ number_format($taxaConformidade ?? 85.4, 1) }}%</p>
                    <div class="flex items-center mt-2">
        ...
        <!-- Períodos Ativos -->
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-600" x-data="{ active: {{ $periodosAtivos ?? 3 }} }">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-sm font-medium text-slate-600 dark:text-slate-300">Períodos Ativos</h3>
                    <p class="text-3xl font-bold text-slate-900 dark:text-white mt-2" x-text="active">{{ $periodosAtivos ?? 3 }}</p>
                    <div class="flex items-center mt-2">
        ...
        <!-- Itens Não Conformes -->
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-600" x-data="{ nonConform: {{ $itensNaoConformes ?? 24 }} }">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-sm font-medium text-slate-600 dark:text-slate-300">Itens Não Conformes</h3>
                    <p class="text-3xl font-bold text-slate-900 dark:text-white mt-2" x-text="nonConform">{{ $itensNaoConformes ?? 24 }}</p>
                    <div class="flex items-center mt-2">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700 rounded-lg">
                        ...
                        <div class="text-right">
                            <p class="text-lg font-bold text-red-600 dark:text-red-400">{{ $item['count'] ?? 12 }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-600">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div>
                                        <div class="font-medium text-gray-900 dark:text-white">{{ $diagnostico['unidade'] ?? 'Hospital Central' }}</div>
                                        <div class="text-sm text-slate-500 dark:text-slate-400">{{ $diagnostico['codigo'] ?? 'HC-001' }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div>
                                        <div class="font-medium text-gray-900 dark:text-white">{{ $diagnostico['setor'] ?? 'UTI Geral' }}</div>
                                        <div class="text-sm text-slate-500 dark:text-slate-400">{{ $diagnostico['subsetor'] ?? 'Medicamentos' }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @endphp
                                    <span class="font-medium {{ $confClass }}">{{ $conformidade }}%</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm">
                                        <div class="text-gray-900 dark:text-white">{{ $diagnostico['data'] ?? '23/05/2025' }}</div>
                                        <div class="text-slate-500 dark:text-slate-400">{{ $diagnostico['hora'] ?? '14:30' }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex space-x-2">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-600">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-start space-x-3">
                        ...
                        <div class="flex-1">
                            <p class="text-sm font-medium text-slate-900 dark:text-white">{{ $atividade['titulo'] ?? 'Diagnóstico concluído' }}</p>
                            <p class="text-xs text-slate-600 dark:text-slate-300">{{ $atividade['descricao'] ?? 'UTI Geral - Hospital Central' }}</p>
                            <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">{{ $atividade['tempo'] ?? '5 min atrás' }}</p>
                        </div>
                    </div>
                    @empty
                    <div class="flex items-start space-x-3">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-600">
                ...
                <div class="space-y-3">
                    @forelse($problemasFrequentes ?? [] as $problema)
                    <div class="flex items-center justify-between">
                        <div class="flex-1">
                            <p class="text-sm font-medium text-slate-900 dark:text-white">{{ $problema['nome'] ?? 'Falta de medicamentos' }}</p>
                            <div class="w-full bg-slate-200 dark:bg-slate-600 rounded-full h-2 mt-1">
                                <div class="bg-danger-500 h-2 rounded-full" style="width: {{ $problema['porcentagem'] ?? 45 }}%"></div>
                            </div>
                        </div>
                        <span class="text-sm font-medium text-slate-900 dark:text-white ml-3">{{ $problema['ocorrencias'] ?? 12 }}</span>
                    </div>
                    @empty
                    <div class="flex items-center justify-between">
                        <div class="flex-1">
