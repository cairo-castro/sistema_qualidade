        ...
        <div class="flex items-center justify-between">
            <div>
              ...
              <div class="flex items-center gap-3">
                @can('create users')
                    <a href="{{ route('admin.users.create') }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center shadow-sm transition-colors">
        ...
        <div class="gqa-card">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 dark:text-gray-100 mt-2">{{ $users->total() }}</p>
                    <div class="flex items-center mt-2">
                        <svg class="w-4 h-4 text-green-500 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 11l5-5m0 0l5 5m-5-5v12"></path>
                        </svg>
                        <span class="text-sm text-green-600 dark:text-green-400">+{{ \App\Models\User::whereDate('created_at', today())->count() }} hoje</span>
                    </div>
                </div>
        ...
        <div class="gqa-card">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 dark:text-gray-100 mt-2">{{ \App\Models\User::whereDate('updated_at', '>=', now()->subDays(30))->count() }}</p>
                    <div class="flex items-center mt-2">
        ...
        <div class="gqa-card">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 dark:text-gray-100 mt-2">{{ $roles->count() }}</p>
                    <div class="flex items-center mt-2">
                        <span class="text-sm text-purple-600 dark:text-purple-400">{{ \App\Models\User::role('Admin')->count() }} Admins</span>
                    </div>
                </div>
        ...
        <div class="gqa-card">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 dark:text-gray-100 mt-2">{{ \App\Models\User::whereDate('created_at', '>=', now()->subDays(7))->count() }}</p>
                    <div class="flex items-center mt-2">
    ...
    <div class="gqa-card">        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-4">
                ...
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200">
                    {{ $users->total() }} {{ $users->total() == 1 ? 'registro' : 'registros' }}
                </span>
            </div>
  
        </div>          @if($users->count() > 0)
            <!-- DataTable Container -->
                                ...
                                <div class="py-1">
                                    ...
                                    @foreach($roles as $role)
                                        <button type="button" 
                                                ...
                                                onclick="bulkAction('assign_role', '{{ $role->id }}')">
                                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @foreach($users as $user)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm">
                                            <div class="text-gray-900 dark:text-gray-100 font-medium" 
                                                 title="{{ $user->updated_at->format('d/m/Y H:i:s') }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @elseif($user->updated_at > now()->subDays(30))
                    ...
                    <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-gray-200 dark:border-gray-700">
                        <div>
                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                ...
                                de
                                <span class="font-semibold text-gray-800 dark:text-gray-200" data-hs-datatable-info-length="">{{ $users->count() }}</span>
                                resultados
                            </p>
                        </div>
            ...
            <div class="text-center py-16">
                <div class="max-w-md mx-auto">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 mb-6">
                        @if(request()->hasAny(['search', 'role', 'activity', 'sort_by']))
                        ...
                        @can('create users')
                            <a href="{{ route('admin.users.create') }}" 
                               class="hospital-btn hospital-btn-primary">
                                ...
                                {{ request()->hasAny(['search', 'role', 'activity', 'sort_by']) ? 'Criar Usuário' : 'Criar Primeiro Usuário' }}
                            </a>
                        @endcan
                    </div>
...
<!-- Bulk Actions Form -->
@can('delete users')
<form id="bulkActionForm" method="POST" action="{{ route('admin.users.bulk-action') }}" class="hidden">
    @csrf
